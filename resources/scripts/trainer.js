...
function getTrainerCalendar(currentMonth, currentYear){
    ...
    //get today's date to be able to disable buttons in currentMonth before today's date
    let todayDate;
    ...
    while(d.getMonth() == mon) {
        if(d.getMonth() < today.getMonth()){
            //if before the current month, disable button
        ...
        else if(d.getDate() < todayDate) {
            //if date is in the current month but before today's date, disable button
            calendarTable += "<td><button disabled style=\"background-color: #808080\">" + d.getDate() + "</button></td>"; 
            if(d.getDay() == 6) { //if last day of week, new table row
                calendarTable += "</tr><tr>";
            }
...
function showEditAvailabilityModal(selectedDate){
    ...
    fetch(apptApiUrl).then(function(response){
        ...
        //get the maxID of all appoinments to be able to add more rows later regardless of if there are existing appointments that day or not
        // const maxIDApiUrl = "https://localhost:5001/api/Appointment/GetMaxID";
        const maxIDApiUrl = "https://trainlikeachampion-g1-api.herokuapp.com/api/Appointment/GetMaxID";
        fetch(maxIDApiUrl).then(function(response){
            console.log(response);
            return response.json();
        }).then(function(json){
            let count = parseInt(json) + 1; //set count to 1 greater than the maxID

            //get formattedDate to show in the top of the modal
            ...
            html += "<h1 class=\"modal-header text-center\">Edit Availability for "+formattedDate+"</h1><br>";
            ...
            if(apptArray.length > 0){
                ...
                let threeRows = count+3; //threeRows so we can print 3 rows
                for(let i = count; i < threeRows; i++){
                    ...
                    html += "<td><select id=activity-"+i+" name=\"activities\" onchange=\"updateApptPrice("+i+","+trainerId+")\" value=\"\" ><option disabled id=\"carOpt"+i+"\" value=\"1\">Cardio</option><option disabled id=\"stOpt"+i+"\" value=\"2\">Strength Training</option><option disabled id=\"kbOpt"+count+"\" value=\"3\">Kickboxing</option><option disabled id=\"yoOpt"+count+"\" value=\"4\">Yoga</option></select>"; //activity, START DISABLED
                    ...
                    count++; //increment count as with i so it stays updated to use in the addRow() method down below
                    updateDisabledActivities(i);
                }
            }
            //set up table close
            html += "</tbody></table>";
            //add a div with centered button for "add more rows", onclick call AddMoreRows(count) to be able to populate more rows
            ...
            html += "</div></div>"; //close modal-content and modal-dialog divs
...
function getActivityCellsForAppt(apptArray, i, trainerId){
    ...
    if(apptArray[i].activityId == 1){ //if cardio, select it and disable the rest
        html += "<option selected value=\"1\" id=\"carOpt"+apptArray[i].apptID+"\">Cardio</option><option disabled id=\"stOpt"+apptArray[i].apptID+"\" value=\"2\">Strength Training</option><option disabled value=\"3\" id=\"kbOpt"+apptArray[i].apptID+"\">Kickboxing</option><option disabled value=\"4\" id=\"yoOpt"+apptArray[i].apptID+"\">Yoga</option></select></td>"; //activity, START DISABLED"
    }
    if(apptArray[i].activityId == 2){ //if strengthTraining, select it and disable the rest
        html += "<option disabled value=\"1\" id=\"carOpt"+apptArray[i].apptID+"\">Cardio</option><option selected id=\"stOpt"+apptArray[i].apptID+"\" value=\"2\">Strength Training</option><option disabled value=\"3\" id=\"kbOpt"+apptArray[i].apptID+"\">Kickboxing</option><option disabled value=\"4\" id=\"yoOpt"+apptArray[i].apptID+"\">Yoga</option></select></td>"; //activity, START DISABLED
    }
    if(apptArray[i].activityId == 3){ //if kickboxing, select it and disable the rest
        html += "<option disabled value=\"1\" id=\"carOpt"+apptArray[i].apptID+"\">Cardio</option><option disabled id=\"stOpt"+apptArray[i].apptID+"\" value=\"2\">Strength Training</option><option selected value=\"3\" id=\"kbOpt"+apptArray[i].apptID+"\">Kickboxing</option><option disabled value=\"4\" id=\"yoOpt"+apptArray[i].apptID+"\">Yoga</option></select></td>"; //activity, START DISABLED
    }
    if(apptArray[i].activityId == 4){ //if yoga, select it and disable the rest
        html += "<option disabled value=\"1\" id=\"carOpt"+apptArray[i].apptID+"\">Cardio</option><option disabled id=\"stOpt"+apptArray[i].apptID+"\" value=\"2\">Strength Training</option><option disabled value=\"3\" id=\"kbOpt"+apptArray[i].apptID+"\">Kickboxing</option><option selected value=\"4\" id=\"yoOpt"+apptArray[i].apptID+"\">Yoga</option></select></td>"; //activity, START DISABLED
    }
    return html;
}
...
function addRow(count, selectedDate){
    ...
    rowHtml += "<td><select id=activity-"+count+" name=\"activities\" onchange=\"updateApptPrice("+count+","+trainerId+")\" value=\"\"><option disabled id=\"carOpt"+count+"\" value=\"1\">Cardio</option><option disabled id=\"stOpt"+count+"\" value=\"2\">Strength Training</option><option disabled id=\"kbOpt"+count+"\" value=\"3\">Kickboxing</option><option id=\"yoOpt"+count+"\" disabled value=\"4\">Yoga</option></select></td>"; 
...
function getTrainerProfileForm(trainer){
    ...
    fetch(activityApiUrl).then(function(response){
        ...
        //update the price field to the price trainer said they would charge for the activity
        for(var i in json){
            if(json[i].activityId == document.getElementById("cardioSelect").value){
            ...
            if(json[i].activityId == document.getElementById("stSelect").value){
            ...
            if(json[i].activityId == document.getElementById("kbSelect").value){
            ...
            if(json[i].activityId == document.getElementById("yogaSelect").value){
...
function trainerEditProfile(){
    ...
    fetch(trainerApiUrl).then(function(response){
        ...
        if(document.getElementById("currPassword").value == undefined){
            ...
            //set up a bodyObject for trainer, then do a PUT with the updated object
            ...
            .then(function(response){
                ...
                fetch(getTrainerApiUrl).then(function(response){
                    ...
                    document.getElementById("trnUpdateSuccessMsg").style.display = "block"; //display success message
                    document.getElementById("newPassword").value = ""; //clear out new password value
                    document.getElementById("newEmail").value = ""; //clear out new email value
                }).catch(function(error){
                    console.log(error);
                })
